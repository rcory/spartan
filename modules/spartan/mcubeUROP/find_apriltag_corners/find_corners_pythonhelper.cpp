...
//int find_corners_pythonhelper(int argc, char** argv) {
void find_corners_pythonhelper(double* corners_list, char* filename) {
  ...
  opts.params.newQuadAlgorithm = 1;
      ...
      for (size_t i=0; i<detections.size(); ++i) {
        ...
        if (d.id == 0) {
              //  std::cout << " - Detection: id = " << d.id << ", "
                //        << "code = " << d.code << ", "
                  //      << "rotation = " << d.rotation << ", "
                //<< "image corners = " << d.p[0] << ", " << d.p[1] << ", " << d.p[2] << ", " << d.p[3] << "\n";

           // for (int j = 0; j < 4 ; j++) { // return corners
             //   corners_list[2*j] = d.p[j].x;
             //   corners_list[2*j+1] = d.p[j].y; 
            //}
            //corners_list[8] = d.cxy.x;
            //corners_list[9] = d.cxy.y;
            //corners_list[0] = d.cxy.x; // only return centers
            //corners_list[1] = d.cxy.y;
            corners_list[0] = d.p[0].x; // only return corners
            corners_list[1] = d.p[0].y;
        }
